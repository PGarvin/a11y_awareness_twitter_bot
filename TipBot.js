...
var statuses = require('./statuses.js').statuses;

statuses.sort(function(a, b) {

  var aConcat = Number(a["times_used"]);
  var bConcat = Number(b["times_used"]);
...
var mostTimesUsed = statuses[0].times_used;

console.log(`\n\n\n***\n\n\n${mostTimesUsed}\n\n\n***\n\n\n`)

statuses.sort(function(a, b) {

  var aConcat = Number(a["next_up"]);
  var bConcat = Number(b["next_up"]);
...
`);
    ...
    this.client.get('statuses/user_timeline', params, (error, tweets, response) => {
      let tweetsArray = this.tweetsArray;
      if (tweetsArray === undefined) {
        console.log(`\n*\n*\n*\n UNDEFINED \n*\n*\n*`);
        tweetsArray = [];
        //tweetsArray.length = tweets.length;
        for (var zz = 0; zz < tweets.length; zz++) {
          tweetsArray.push(tweets[zz]);
        }
        this.tweetsArray = tweetsArray;
      }
      if (tweetsArray !== undefined) {
        //console.log(tweetsArray.length, tweetsArray[0]);

        //this.lastTimelineTweetId = tweetsArray[0].id_str;
      }
      this.lastTimelineTweetId = tweetsArray[0].id_str;
      for (var i=0; i < tweetsArray.length; i++) {
        for (var j=0; j < statuses.length; j++) {
          if (tweetsArray[i].id_str === statuses[j].id_str) {
            statuses[j].favorite_count = tweetsArray[i].favorite_count;
            statuses[j].retweet_count = tweetsArray[i].retweet_count;
          }
        }
      }

      let now = new Date();
      let lastTweet = new Date(JSON.stringify(tweetsArray[0].created_at));
      ...
      if (timeDistance > timeBetweenPosts) {
      ...
      if (statusNumber !== 0) {
      for (var jk = 0; jk < statusNumber; jk++) {
      statuses[jk].frequency = statuses[jk].frequency + Number(Math.random() * 0.1);
      }
      }

      var content = `var statuses = [${JSON.stringify(statuses[0])}`;
      ...
      /*console.log(`var statuses = [${JSON.stringify(statuses[0])},\n${JSON.stringify(statuses[1])}]`);
      */
